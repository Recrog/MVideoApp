...
import_electron.ipcMain.handle("ffmpeg:exportTimeline", async (_e, items) => {
  ...
  for (const it of items) {
    if (it.type === "image") {
      const clip = import_node_path.default.join(dir, `img_${clipPaths.length}.mp4`);
      ...
      const clip = import_node_path.default.join(dir, `vid_${clipPaths.length}.mp4`);
      ...
      if (it.overlayAudioPath || it.overlays && it.overlays.length) {
        ...
        const totalInputs = 1 + overlayInputs.length;
        const audioInputs = overlayInputs.length + 1;
  ...
  const listPath = import_node_path.default.join(dir, "concat.txt");
  await import_promises.default.writeFile(listPath, clipPaths.map((p) => `file '${p.replace(/'/g, "'\\''")}'`).join("\n") + "\n");
